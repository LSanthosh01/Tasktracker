...
router.put('/:id/review', protect, authorize('admin', 'manager'), async (req, res) => {
  try {
    ...
    if (report.submittedBy && report.submittedBy.role === 'manager' && req.user.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Only admin can review and approve reports submitted by managers' });
    }
